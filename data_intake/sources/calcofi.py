...
def run(ctx: IngestionContext) -> dict:
    ...
    def worker(item: dict, position: int) -> dict:
        ...
        # For now, page entries are discovery artifacts, not raw data files.
        # Do not pretend they are CSVs.
        if kind == "page":
            ...
            record = SourceFileRecord(
                ...
                validation={
                    "local": {
                        ...
                        "warnings": ["Page URL saved as discovery artifact, not treated as raw data."],
                        "errors": [],
                    }
                },
            )
        ...
        try:
            summary = summarize_file(local_path) if local_path.suffix.lower() != ".zip" else {
                "file_type": "zip",
                "size_bytes": local_path.stat().st_size,
                "filename": local_path.name,
                "source_name": SOURCE_NAME,
